...
type AwsS3CollectionState struct {
	collection_state.ArtifactCollectionStateImpl[*AwsS3BucketSourceConfig]
...
func (s *AwsS3CollectionState) Init(config *AwsS3BucketSourceConfig, path string) error {
	...
	// call base init
	return s.ArtifactCollectionStateImpl.Init(config, path)
}

// IsEmpty returns true if the collection state is empty
func (s *AwsS3CollectionState) IsEmpty() bool {
	return s.StartAfterKey == nil && s.ArtifactCollectionStateImpl.IsEmpty()
}

// OnCollected adds new/updates an existing object with its current metadata
// Note: the object name is the full path to the object
func (s *AwsS3CollectionState) OnCollected(metadata collection_state.SourceItemMetadata) error {
	err := s.ArtifactCollectionStateImpl.OnCollected(metadata)
	if err != nil {
		return err
	}
	// TODO KAI FIX ME
	//if s.UseStartAfterKey {
	//	s.StartAfterKey = &metadata.LocalName
	//}
	return nil
}
