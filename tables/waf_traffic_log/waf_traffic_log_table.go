...
func (c *WafTrafficLogTable) GetSourceMetadata() []*table.SourceMetadata[*WafTrafficLog] {
	...
	defaultS3ArtifactConfig := &artifact_source_config.ArtifactSourceConfigImpl{
		FileLayout: utils.ToStringPointer("AWSLogs/(%{DATA:org_id}/)?%{NUMBER:account_id}/WAFLogs/%{DATA:cloudfront_or_region}/%{DATA:cloudfront_name_or_resource_name}/%{YEAR:year}/%{MONTHNUM:month}/%{MONTHDAY:day}/%{HOUR:hour}/%{MINUTE:minute}/%{DATA}.gz"),
	}
...
func (c *WafTrafficLogTable) EnrichRow(row *WafTrafficLog, sourceEnrichmentFields schema.SourceEnrichment) (*WafTrafficLog, error) { // initialize the enrichment fields to any fields provided by the source
	...
	row.TpIndex = strings.Split(*row.WebAclId, ":")[4]
